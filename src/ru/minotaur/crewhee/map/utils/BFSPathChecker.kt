...
class BFSPathChecker(val mapMatrix: Array<IntArray>) {
    ...
    fun initPathChecker(start : Point, end : Point? = null) {
    ...
    fun getPathLength() : Array<IntArray> {
        return pathLength
    }
    ...
    fun checkMap() : Boolean {
        ...
        if (!res) {
            for (y in 0 until pathLength[0].size) {
                for (x in 0 until pathLength.size) {
                    print("%3d".format(pathLength[x][y]))
                }
                print('\n')
            }
        }
